...
`編集` を押すと、現在の投句が1行1句で入った編集画面が開きます。行を書き換えると変更、行を増やすと追加、行を消すと削除になります。入力欄を空にして送信すると、登録済みの投句をすべて削除します。空行は無視されます。
...
句会案内チャンネルの「参加の記録」から、対象、表示範囲、表示軸、俳号、Discord上の表示件数を選べます。表示件数は1以上の整数で指定し、空欄なら全件が表示対象です。Embedには収まる範囲で投句と選句の詳細が表示され、選評・総評や収まらない内容は全件Markdownで自分だけに表示されます。

サーバーの参加記録公開設定が `private` の場合は自分の記録だけを表示します。`guild_public` の場合は同じサーバーの他参加者も選べますが、他人について表示されるのは、そのサーバー内で結果公開済みの句会だけです。
...
管理パネルでは、投句一覧公開後に「投句一覧PDFを送信」、結果公開後に「結果PDFを送信」を利用できます。送信前の確認画面では無記名が初期選択され、作者公開済みの場合だけ記名を選べます。PDFは句会の開催チャンネルへ投稿されます。作者公開方式が手動の場合は、同じ管理パネルから作者公開も実行できます。
