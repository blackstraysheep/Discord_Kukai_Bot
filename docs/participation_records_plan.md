...
- 同一Discordユーザーが複数の俳号を使っている場合に、俳号ごと・サーバごとの表示や絞り込みができるようにする。

ここでいう参加記録は、句会への参加、投句、選句、選評、総評、参加時の俳号のうちDBに保存されているものを指す。
...
- 俳号は句会ごとの参加者名として扱い、同一ユーザーの固定プロフィールとはみなさない。
- 俳号が同じでも別ユーザーなら別人として扱う。俳号別表示は、必ずDiscord `user_id` で絞った後の分類軸とする。
- 重複判定は正規化済み本文の完全一致のみ。
...
- 句会タイトルの近くに、その句会で使った俳号とサーバ名を表示する。
...
初期実装から、本人記録では次の表示軸を選べるようにする。

- `group_by=kukai`: 句会ごとに時系列で表示する。既定値。
- `group_by=server`: サーバごとにまとめ、その下に句会ブロックを表示する。
- `group_by=haigo`: 俳号ごとにまとめ、その下に句会ブロックを表示する。

他人記録は現在サーバ内に限定されるため、初期実装では `group_by=kukai` / `group_by=haigo` のみ対応すればよい。

俳号が未設定の場合は、分類上は `（俳号未設定）` として扱う。表示名としては既存方針どおり、サーバー表示名または `UID:{user_id}` にフォールバックする。

### 絞り込み

本人記録では、将来の検索性を考えて次の絞り込みをサービス層で扱える設計にする。

- `server_id`: 特定サーバに限定する。Discordコマンド上の入力方法は初期実装では省略してもよい。
- `haigo`: 完全一致で俳号を絞り込む。

Discord slash commandの選択肢としてサーバ一覧や俳号一覧を動的に出す場合は、後続改善でautocompleteを追加する。初期実装では `scope` と `group_by` を優先し、`haigo` 絞り込みは文字列オプションとして実装する。
...
- `group_by=server` / `group_by=haigo` を指定しても、閲覧権限は本人記録の範囲を超えない。
...
- `group_by=kukai/server/haigo` と `haigo` 絞り込みを適用する。
- 本人閲覧と他人閲覧の表示許可を判定する。
...
対象ユーザーがその句会で使った俳号は、次の優先順で解決する。

1. `entries.haigo`
2. `kukai_participants.haigo`
3. `None`

俳号別表示・俳号絞り込みでは、この解決結果を使う。Discord表示名にフォールバックした値で俳号分類しない。
...
- `/record me scope: current|all group_by: kukai|server|haigo haigo: limit:`
- `/record user user: group_by: kukai|haigo haigo: limit:`
...
- `all` は同一DB内の全サーバを見る。
- `group_by=server` はサーバごとにまとめる。
- `group_by=haigo` は参加時俳号ごとにまとめる。
...
```text
⚠️ 以前の参加記録に同じ投句があります。
- 第1回〇〇句会（俳号: 春月 / サーバ: 俳句サロン）
- 第3回△△句会（俳号: 青嵐 / サーバ: 夜句会）
```

句会タイトルは可能ならリンクにする。

重複警告は同一Discord `user_id` の全履歴を見るため、俳号が違っても警告する。警告文には、そのとき使っていた俳号とサーバ名を添えて、本人が判断できるようにする。
...
- `group_by=server` でサーバごとにまとまる。
- `group_by=haigo` で参加時俳号ごとにまとまる。
- `haigo` 指定時、その俳号の記録だけ返る。
- 俳号未設定は `（俳号未設定）` として分類される。
...
- 同じ正規化済み本文を本人が別俳号で投句済みでも警告が出る。
- 同じ正規化済み本文を本人が同一サーバの別句会で投句済みなら警告が出る。
- 重複警告に過去句会の俳号とサーバ名が含まれる。
- 他人の同一本文は警告対象にならない。
...
- 俳号は句会ごとに変わり得るため、ユーザー同一性の根拠にしない。俳号は表示・分類・絞り込みの軸としてだけ使う。
